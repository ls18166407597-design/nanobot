...
class GitHubTool(Tool):
        ...
        "properties": {
            "action": {
                ...
                "enum": [
                    ...
                    "list_prs", "get_pr_diff", "list_repos", "setup"
                ],
            ...
            "body": {
                "type": "string",
                "description": "Body content for issue/comment."
            },
            "limit": {
                "type": "integer",
            ...
            "setup_token": {
                "type": "string",
                "description": "GitHub PAT for setup."
            }
        },
        "required": ["action"]
    }
    ...
    async def execute(self, action: str, **kwargs: Any) -> str:
        ...
        try:
            ...
            if action == "list_issues":
                ...
                if not repo_name: return "Error: 'repo' is required."
                return self._list_issues(g, repo_name, kwargs.get("state", "open"), kwargs.get("limit", 10))
            
            elif action == "get_issue":
                repo_name = kwargs.get("repo")
                number = kwargs.get("issue_number")
                if not repo_name or not number: return "Error: 'repo' and 'issue_number' required."
                ...
                if not repo_name or not title: return "Error: 'repo' and 'title' required."
                ...
                number = kwargs.get("issue_number")
                body = kwargs.get("body")
                if not repo_name or not number or not body: return "Error: 'repo', 'issue_number', 'body' required."
                return self._comment_issue(g, repo_name, number, body)

            elif action == "list_prs":
                repo_name = kwargs.get("repo")
                if not repo_name: return "Error: 'repo' is required."
                return self._list_prs(g, repo_name, kwargs.get("state", "open"), kwargs.get("limit", 10))
    ...
    def _get_issue(self, g, repo_name, number):
        ...
        output = [f"#{issue.number} {issue.title}", f"State: {issue.state}", f"Author: {issue.user.login}", f"\nBody:\n{issue.body}\n", f"Comments ({len(comments)}):"]
    ...
    def _list_repos(self, g, limit):
        ...
        for repo in g.get_user().get_repos(sort="updated", direction="desc"):
            if count >= limit: break
            output.append(f"{repo.full_name} ({repo.stargazers_count}★) - {repo.description or 'No desc'}")
            count += 1
        return "\n".join(output)
