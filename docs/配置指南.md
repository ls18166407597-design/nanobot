...
- **忙碌提示阈值**: `tools.busyNoticeThreshold`
- **忙碌提示节流**: `tools.busyNoticeDebounceSeconds` (秒)
- **错误回传兜底通道**: `tools.errorFallbackChannel`
- **错误回传兜底 Chat ID**: `tools.errorFallbackChatId`
- **工具白/黑名单**: `tools.enabledTools` / `tools.disabledTools`
...
### 4. 本地 Gemini 桥接 (Local Bridge)

如果您正在使用本地提供的 OpenAI 兼容接口（如 `8045/8046` 端口），可以直接在 `config.json` 中配置：
```
{
  "providers": {
    "openai": {
...
- macOS Vision 权限
- 网络连通性
注意：默认单代理直接调用 `browser`，仅在长时或并行任务时使用子任务。

## 🧩 错误回传兜底配置 (可选)
当 system 消息缺少 origin 时，错误回传会走兜底通道。可在 `config.json` 中配置：
```json
{
  "tools": {
    "errorFallbackChannel": "cli",
    "errorFallbackChatId": "direct"
  }
}
```
...
## 🧰 工具配置文件 (可选)
以下工具需要在 `NANOBOT_HOME` 下提供配置文件：
- `tavily_config.json`（keys 或 key）
- `tianapi_config.json`（key）
- `weather_config.json`（key，host 可选）
- `tushare_config.json`（token）
- `feishu_config.json`（app_id, app_secret）
