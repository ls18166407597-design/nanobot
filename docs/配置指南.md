...
```bash
nanobot config set tools.enabled_tools '["read_file","write_file","edit_file","list_dir","exec","browser","tavily","duckduckgo","github","message","mac_control","mac_vision","task","skills","memory"]'
```

火车票能力建议启用 `train_ticket`（由工具内部调用 12306 MCP 后端）：

```bash
nanobot config set tools.enabled_tools '["read_file","write_file","edit_file","list_dir","exec","browser","tavily","duckduckgo","github","train_ticket","message","mac_control","mac_vision","task","skills","memory"]'
```
