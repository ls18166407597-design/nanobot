...
- 设置是否暴露通用 MCP 工具（默认 false）：`nanobot config set tools.mcp.expose_direct false`
- 设置联网默认入口：`nanobot config set tools.policy.web_default "tavily"`（可选：`browser`）
- 设置文件写入策略开关：`nanobot config set tools.file_write_policy.enabled true`
- 设置受控文件确认开关：`nanobot config set tools.file_write_policy.require_confirm_for_controlled true`
...
- 默认不向模型暴露通用 `mcp` 工具；推荐通过领域工具（如 `github`、`train_ticket`）间接使用 MCP 后端。

精简工具列表（推荐）：

```bash
nanobot config set tools.enabled_tools '["read_file","write_file","edit_file","list_dir","exec","browser","tavily","mail","gmail","qq_mail","github","message","mac_control","mac_vision","task","skills","memory"]'
```

火车票能力建议启用 `train_ticket`（由工具内部调用 12306 MCP 后端）：

```bash
nanobot config set tools.enabled_tools '["read_file","write_file","edit_file","list_dir","exec","browser","tavily","mail","gmail","qq_mail","github","train_ticket","message","mac_control","mac_vision","task","skills","memory"]'
```
...
- `tools.file_write_policy.read_only_patterns`：只读文件匹配规则（默认含 `workspace/IDENTITY.md`、`workspace/AGENTS.md`、`.home/config.json`、`nanobot/config/*.py`）。
- `tools.file_write_policy.controlled_patterns`：受控文件匹配规则（默认含 `workspace/TOOLS.md`、`workspace/USER.md`、`docs/**/*.md` 等）。
- 受控文件默认要求工具参数中显式带 `confirm=true` 且 `change_note` 非空。

示例：

```bash
nanobot config set tools.file_write_policy.read_only_patterns '["workspace/IDENTITY.md","workspace/AGENTS.md",".home/config.json"]'
nanobot config set tools.file_write_policy.controlled_patterns '["workspace/TOOLS.md","docs/**/*.md"]'
```
