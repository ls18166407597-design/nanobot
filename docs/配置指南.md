...
- 设置联网默认入口：`nanobot config set tools.policy.web_default "tavily"`（可选：`duckduckgo`、`browser`）
...
- 默认不向模型暴露通用 `mcp` 工具；推荐通过领域工具（如 `github`、`train_ticket`、`duckduckgo`）间接使用 MCP 后端。

精简工具列表（推荐）：

```bash
nanobot config set tools.enabled_tools '["read_file","write_file","edit_file","list_dir","exec","browser","tavily","duckduckgo","mail","gmail","qq_mail","github","message","mac_control","mac_vision","task","skills","memory"]'
```

火车票能力建议启用 `train_ticket`（由工具内部调用 12306 MCP 后端）：

```bash
nanobot config set tools.enabled_tools '["read_file","write_file","edit_file","list_dir","exec","browser","tavily","duckduckgo","mail","gmail","qq_mail","github","train_ticket","message","mac_control","mac_vision","task","skills","memory"]'
```
