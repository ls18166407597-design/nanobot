...
- 工具配置目录：`.home/tool_configs/`（新）

工具配置兼容策略：

- 读取时：优先从 `.home/tool_configs/` 读取；不存在时回退 `.home/` 旧位置。
- 写入时：统一写入 `.home/tool_configs/`。
- 一键迁移：`nanobot migrate-tool-configs`
- 迁移预览：`nanobot migrate-tool-configs --dry-run`
